...
function _oidcStartAuth(op, rememberMe) {
  _oidcReset();
  _oidcState.state_msg = 'RequestingAccountAuth';
  // Open popup synchronously during user gesture to avoid popup blockers
  _oidcState.popup = window.open('about:blank', 'oidc_auth');
  _oidcStartAuthAsync(op, rememberMe);
}

async function _oidcStartAuthAsync(op, rememberMe) {
  ...
  try {
    ...
    if (!resp.ok) {
      _oidcState.failed_msg = `HTTP ${resp.status}`;
      if (_oidcState.popup) _oidcState.popup.close();
      return;
    }
    const data = await resp.json();
    if (_oidcState.cancelled) return;
    if (!data.url) {
      _oidcState.failed_msg = data.code || 'No auth URL returned';
      if (_oidcState.popup) _oidcState.popup.close();
      return;
    }
...
window.setByName = (name, value) => {
  switch (name) {
    ...
    case 'account_auth': {
      ...
      _oidcStartAuth(auth.op, auth.remember);
      break;
    }
    case 'account_auth_cancel':
      _oidcState.cancelled = true;
      break;
    default:
      break;
  }
...
function _getByName(name, arg) {
  switch (name) {
    ...
    case 'account_auth_result':
      return JSON.stringify({
        state_msg: _oidcState.state_msg,
        failed_msg: _oidcState.failed_msg,
        url: _oidcState.url,
        auth_body: _oidcState.auth_body,
      });
  }
  return '';
}
